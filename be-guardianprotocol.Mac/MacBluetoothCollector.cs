...
{
    public class MacBluetoothCollector : ISignalCollector
    {
        ...
        private async Task<bool> CheckBluetoothStatus()
        {
            try
            {
                var output = await RunCommandAsync("system_profiler", "SPBluetoothDataType | grep 'State: On'");
        ...
        private async Task<int> GetConnectedDevices()
        {
            try
            {
                var output = await RunCommandAsync("system_profiler", "SPBluetoothDataType | grep 'Connected: Yes' | wc -l");
